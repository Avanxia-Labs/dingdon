...
// Extend Window interface to include ChatbotLoaded
declare global {
    interface Window {
...
// Define ActiveChat type if not imported from elsewhere
...
import { Send, Wifi, WifiOff, RefreshCcw, Pause, Play, FileText, Tag, UserPlus, Info, Users, User, Bot } from "lucide-react";
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ workspaceId }) => {
    ...
    }, [socket, setActiveChat, addMessageToActiveChat, session?.user?.name, setActiveBotConfig]);
        ...
        <div className="flex h-full relative flex-col sm:flex-row">
            ...
            <div className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 sm:border-r border-b sm:border-b-0 bg-white p-2 sm:p-3 md:p-4 flex flex-col min-w-0 h-1/3 sm:h-full">
                ...
                <h2 className="text-sm sm:text-base md:text-lg font-bold mb-2 md:mb-3 text-green-700">
                    📥 <span className="hidden sm:inline">{String(safeTranslate("chatPanel.requestsTitle"))}</span>
                ...
                <div className="space-y-1 sm:space-y-2 mb-3 md:mb-4 max-h-48 sm:max-h-64 overflow-y-auto">
                    {requests.map((req) => (
                        ...
                        >
                            <p className="font-semibold text-green-800 text-xs sm:text-sm">
                                🆕 <span className="hidden sm:inline">{String(safeTranslate("chatPanel.sessionLabel"))}: </span>{req.sessionId.slice(-6)}
                            </p>
                    ...
                    {requests.length === 0 && (
                        <p className="text-gray-500 text-sm mt-2 italic">
                            {String(safeTranslate("chatPanel.noRequests"))}
                        </p>
                    )}
                </div>
                ...
                <h2 className="text-sm sm:text-base md:text-lg font-bold mb-2 md:mb-3 text-blue-700">
                    💬 <span className="hidden sm:inline">{String(safeTranslate("chatPanel.myActiveChatsTitle"))}</span>
                ...
                <div className="space-y-2 flex-1 overflow-y-auto">
                        ...
                        return (
                            ...
                            >
                                <p className="font-semibold text-xs sm:text-sm">
                                    ...
                                    <span className="hidden sm:inline">{String(safeTranslate("chatPanel.sessionLabel"))}: </span>
                                    ...
                                    {isTransferred && (
                                        <span className={`ml-1 sm:ml-2 text-xs px-1 sm:px-2 py-1 rounded-full ${
                                            ...
                                            <span className="hidden sm:inline">{String(safeTranslate("chatPanel.transferredLabel"))}</span>
                                            <span className="sm:hidden">T</span>
                                        </span>
                                    )}
                                </p>
                                <p className={`text-xs sm:text-sm ${isActive ? 'text-gray-200' : 'text-gray-600'} mt-1`}>
                                    {getRealMessageCount(chat.messages)} msgs • {String(safeTranslate(`chatPanel.status.${chat.status}`) || chat.status)}
                                    {isTransferred && chat.transferInfo?.transferredFromAgent && (
                                        <span className="block mt-1 text-xs truncate">
                                            <span className="hidden sm:inline">{String(safeTranslate("chatPanel.transferredFrom", { agent: chat.transferInfo.transferredFromAgent }))}</span>
                    ...
                    {myActiveChats.length === 0 && (
                        <p className="text-gray-500 text-sm mt-2 italic">
                            {String(safeTranslate("chatPanel.noActiveChats"))}
                        </p>
                    )}
                </div>
            </div>
