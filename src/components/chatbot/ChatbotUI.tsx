          ...
          <>
            ...
            <div className="p-3 border-t border-t-gray-300 bg-white rounded-b-lg flex-shrink-0">
                {status === 'closed' ? (
                  ...
                  <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
                      <textarea value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={handleKeyPress} placeholder={t(`chatbotUI.${status === 'in_progress' ? 'inputPlaceholderAgent' : status === 'pending_agent' ? 'inputPlaceholderPending' : 'inputPlaceholder'}`)} className="text-black flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg resize-none focus:outline-none" rows={1} disabled={isLoading || status === 'pending_agent'} />
