...
// Carga keywords desde BD
async function loadKeywords(workspaceId: string, language: string): Promise<Keywords> {
  const { data, error } = await supabaseAdmin
    .from('lead_keywords')
    .select('keyword, category')
    .eq('workspace_id', workspaceId)
...
// Análisis por reglas con keywords
function analyzeByRules(history: Message[], keywords: Keywords) {
  const userMessages = history.filter(m => m.role === 'user');
  const text = userMessages.map(m => m.content.toLowerCase()).join(' ');
  ...
  keywords.hot.forEach((k: string) => {
    if (text.includes(k.toLowerCase())) { 
      score += 30; 
      found.push({ keyword: k, category: 'HOT' }); 
    }
  });
  keywords.warm.forEach((k: string) => {
    if (text.includes(k.toLowerCase())) { 
      score += 15; 
      found.push({ keyword: k, category: 'WARM' }); 
    }
  });
  keywords.cold.forEach((k: string) => {
    if (text.includes(k.toLowerCase())) { 
      score -= 10; 
      found.push({ keyword: k, category: 'COLD' }); 
    }
  });
  ...
  // Generate reasoning
  const hotCount = found.filter(f => f.category === 'HOT').length;
  const warmCount = found.filter(f => f.category === 'WARM').length;
  const coldCount = found.filter(f => f.category === 'COLD').length;
