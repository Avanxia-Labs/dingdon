...
export default function LeadClassificationPage() {
  ...
  const addKeyword = async (type: 'hot' | 'warm' | 'cold', keyword: string) => {
    if (!keyword.trim()) {
      setFeedback(String(safeTranslate('leadClassification.feedback.keywordEmpty', 'Please enter a keyword')));
      setTimeout(() => setFeedback(''), 3000);
      return;
    }
    ...
    if (currentKeywords.includes(normalizedKeyword)) {
      setFeedback(String(safeTranslate('leadClassification.feedback.keywordDuplicate', 'This keyword already exists')));
      setTimeout(() => setFeedback(''), 3000);
      return;
    }
    
    try {
      setFeedback(String(safeTranslate('leadClassification.feedback.savingKeywords', 'Saving keywords...')));
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (keywords.hot.length > 0 || keywords.warm.length > 0 || keywords.cold.length > 0) {
            ...
            setFeedback(String(safeTranslate('leadClassification.feedback.usingDefaultKeywords', 'Using default keywords')));
